...
static TokenType identifierType()
{
    ...
    case 'i':
        if (scanner.current - scanner.start > 1)
        {
            ...
            case 'f':
                ...
                if (scanner.start[2] == '_')
                    return checkKeyword(3, 3, "not", TOKEN_IF_NOT);
    ...
    case 'j':
        return checkKeyword(1, 3, "ump", TOKEN_JUMP);
    case 'l':
        if (scanner.current - scanner.start > 1)
        {
            switch (scanner.start[1])
            ...
            case 'a':
                return checkKeyword(2, 3, "bel", TOKEN_LABEL);
            }
        }
        break;
    ...
    case 'p':
        if (scanner.current - scanner.start > 1)
        {
            ...
            {
            case 'a':
                return checkKeyword(2, 2, "ss", TOKEN_PASS);
            case 'r':
    ...
    case 'v':
        if (scanner.current - scanner.start > 1)
        {
            ...
            case 'e':
                return checkKeyword(2, 4, "ctor", TOKEN_VECTOR);
            }
        }
        break;
    case 'h':
        return checkKeyword(1, 6, "ashmap", TOKEN_HASHMAP);
    case 'w':
...
Token scanToken()
{
    ...
    case '&':
        return match('&') ? makeToken(TOKEN_AND) : makeToken(TOKEN_ERROR);
    case '|':
        return match('|') ? makeToken(TOKEN_OR) : makeToken(TOKEN_ERROR);
